...
export async function runModelTests() {
  ...
  // boardToTensor
  test('boardToTensor — shape [1, 257]', () => {
  ...
  test('boardToTensor — empty board encoding', () => {
    const board = emptyBoard();
  ...
  test('boardToTensor — white piece encoding', () => {
    const board = emptyBoard();
  ...
  test('boardToTensor — black king encoding', () => {
    const board = emptyBoard();
  ...
  test('boardToTensor — throws on wrong size', () => {
    assert.throws(() => boardToTensor([[1, 2, 3]], 1), /expected 64 cells/);
  });

  // createModel
  test('createModel("small") — output shapes', () => {
  ...
  test('createModel("small") — layer sizes 128, 64', () => {
    const model = createModel('small');
  ...
  test('createModel("medium") — layer sizes 256, 128, 64', () => {
  ...
  test('createModel("large") — layer sizes 512, 256, 128, 64', () => {
  ...
  // predict
  test('predict — returns {move, probabilities, value}', async () => {
    const model = createModel('small');
    const board = emptyBoard();
  ...
  test('predict — probabilities sum to ~1', async () => {
    const model = createModel('small');
    const board = emptyBoard();
  ...
  // train
  test('train — returns loss number', async () => {
    const model = createModel('small');
    const batch = [
  ...
  test('train — empty batch returns {loss: 0}', async () => {
    const model = createModel('small');
  ...
  // save / load
  test('saveModel + loadModel — roundtrip weights match', async () => {
    const model = createModel('small');
